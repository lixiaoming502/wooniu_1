...
function showbook(){
	...
	if(books.length){
		...
		for(var i=books.length-1;i>-1;i--){
            ...
            bookhtml+='<a href="/i'+articleid+'/"><p class="title">'+articlename+'</p>';
            ...
            bookhtml+='<p class="author">上次阅读：<a style="color: Red;" href="/i'+articleid+'/'+lastchapterid+'.html" target="_blank">'+lastchapter+'</a></p>';
	...
	$(function(){
		$.ajax({
			...
			success: function(data){
						...
						$("#"+aid+"newcase_lastchapter").html('<a href="/i'+$d[0].replace(/\s/g,'') +'/'+$d[5]+'.html">'+$d[6]+'</a>');
					}
		});
	})
